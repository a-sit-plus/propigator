...
class PropigatorConventionsPlugin : Plugin<Project> {
    override fun apply(target: Project) = with(target) {
        ...
        logger.info("Propigator Conventions Plugin applied to project: ${'$'}{target.path}")
...
class PropigatorConventionsExtension(private val project: Project) {
    init {
    ...
    fun mavenPublish(name: String, description: String) = project.afterEvaluate {
        val javadocJar = setupDokka(
            baseUrl = "https://github.com/a-sit-plus/propigator/tree/main/",
        )
            ...
            publications {
                withType<org.gradle.api.publish.maven.MavenPublication> {
                    ...
                    pom {
                        ...
                        url.set("https://github.com/a-sit-plus/propigator")
                        licenses {
                            license {
                        ...
                        scm {
                            connection.set("scm:git:git@github.com:a-sit-plus/propigator.git")
                            developerConnection.set("scm:git:git@github.com:a-sit-plus/propigator.git")
                            url.set("https://github.com/a-sit-plus/propigator")
                        }
                    }
                }
...
fun Project.propigatorConventions(init: PropigatorConventionsExtension.() -> Unit) {
    PropigatorConventionsExtension(this).init()
}
...
private fun Project.disableWebTestExecutablesWhenNoTests() {
    val hasTestSources = listOf("commonTest", "jsTest", "wasmJsTest")
        .any { sourceSet ->
            !fileTree("src/$sourceSet") {
                include("**/*.kt")
                include("**/*.kts")
            }.isEmpty
        }
    ...
    tasks.configureEach {
        if (name.isJsTestTaskName() || name.isJsTestExecutableTaskName()) {
            enabled = false
        }
    }
}

private fun String.isJsTestTaskName() =
    endsWith("Test") && contains(Regex("[Jj]s"))

private fun String.isJsTestExecutableTaskName() =
    matches(Regex("compileTest.*ExecutableKotlin(?:Wasm)?Js(?:Optimize)?"))
